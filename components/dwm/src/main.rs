...
impl ImageViewerState {
    ...
    pub fn open_image(&mut self, p: &str) {
        ...
        if self.raw_len >= 2 && &self.raw_bytes[0..2] == b"BM" {
            self.img_type = "VladOS Bitmap (.bmp)";
            if self.raw_len >= 26 {
...
impl Gfx {
    ...
    fn draw_action_center(&self, ds: &DesktopState) {
        ...
        self.draw_text(x + 22, n1_y + 8, "VladOS Security", 0x00B7C3, 0x1A1F26);
    ...
    fn draw_desktop(&self) {
        // VladOS 10 Fluent Dark Hero Gradient
        ...
        // Luminous VladOS 10 Fluent 4-tile logo on the right side of desktop
    ...
    fn draw_taskbar(&self, ds: &DesktopState) {
        ...
        // Dynamic Taskbar: VladOS 10 EnumVladOS style
    ...
    fn draw_path_editor(&self, ds: &DesktopState, is_active: bool) {
        ...
        let path_entries = [
            r"C:\VladOS\System32 (Default VladOS System)",
    ...
    fn draw_notepad(&self, np: &NotepadState, is_active: bool) {
        ...
        self.draw_text(wx + 12, sb_y + 4, "Ln 1, Col 1   |   100%   |   VladOS (CRLF)   |   UTF-8", 0x888888, 0x1F1F1F);
    }
...
// Authentic VladOS 10 Aero Cursor (16x22)
...
impl CursorManager {
    ...
    fn draw_at(&mut self, gfx: &Gfx, mx: usize, my: usize) {
        ...
        // 2. Draw VladOS 10 Soft Drop Shadow (multi-level alpha blending)
        ...
        // 3. Draw VladOS 10 Crisp Aero Arrow
        for (cy, row_str) in WIN10_CURSOR.iter().enumerate() {
            for (cx, ch) in row_str.chars().enumerate() {
                let px = mx + cx;
                let py = my + cy;
...
impl TerminalState {
    fn new() -> Self {
        ...
        term.add_line("");
        term.add_line("VladOS 10 GUI Desktop active (explorer.vex).");
    ...
    fn execute_command(
        ...
        if let Ok(cmd) = core::str::from_utf8(&cmd_bytes[..len]) {
            ...
            if eq_ignore_ascii_case(cmd, "help") {
                ...
                self.add_line("PLAY [file]  - Launch VladOS Media Player (MP3/WAV/MP4)");
...
fn launch_vex(path: &str, ds: &mut DesktopState, term: &mut TerminalState) {
    let clean = path.trim();
    let (target_prog, args) = if let Some(idx) = clean.find(' ') {
        (clean[..idx].trim(), clean[idx + 1..].trim())
    } else {
        (clean, "")
    };
    let name = if let Some(idx) = target_prog.rfind('\\').or_else(|| target_prog.rfind('/')) {
        &target_prog[idx + 1..]
    } else {
        target_prog
    };
        ...
        || eq_ignore_ascii_case(name, "control")
    {
        ...
        if n >= 4 && header[0..4] == [0x7F, b'E', b'L', b'F'] {
            term.add_line("[Process Created: VladOS Subsystem GUI / VEX-64]");
...
fn handle_mouse_click(
    ...
    if ds.context_menu_open {
        ...
        if mx >= cm_x && mx < cm_x + cm_w && my >= cm_y && my < cm_y + cm_h {
            ...
            match item_idx {
                ...
                6 => {
                    term.add_line("[Context Menu: Personalize -> VladOS 10 Dark Acrylic Theme]");
                }
                _ => {}
            }
    ...
    // 2. Dynamic Taskbar App Buttons (VladOS 10 EnumVladOS style)
    if my >= tb_y && mx >= 232 {
        let mut open_wins = [0usize; 8];
        let mut open_count = 0;
        for wid in 0..8 {
    ...
    if ds.start_menu_open {
        ...
        if mx < 340 && my >= sm_y && my < tb_y {
            ...
            if mx >= 194 && mx <= 320 && my >= sm_y + 82 && my <= sm_y + 150 {
                ...
                term.add_line("Desktop Shell:             explorer.vex (VladOS 10 Fluent Dark)");
    ...
    // 4. VladOS in Z-Order (Top to Bottom hit-testing)
...
pub extern "C" fn _start() -> ! {
    ...
    // 1. Draw Full Authentic VladOS 10 GUI Desktop
    gfx.draw_desktop();
    gfx.render_windows(&ds, &term);

    // 2. Initialize VladOS 10 Aero Cursor Manager
        ...
        if n > 0 && n <= read_buf.len() {
            ...
            for i in 0..n {
                ...
                if esc_state == 1 {
                    ...
                    esc_state = 0;
                    // Arrow Keys move VladOS 10 cursor smoothly (iPad / keyboard navigation)
                    if b == b'A' {
                        // Up
                ...
                if b == b'\r' || b == b'\n' {
                    ...
                    // Tab auto-completion (with multiple candidate cycling)
                    if term.input_len > 0 {
                        if let Ok(curr) = core::str::from_utf8(&term.input_buf[..term.input_len]) {
                            ...
                            if !last_token.is_empty() {
                                ...
                                for &c in TAB_COMMANDS {
                                    if starts_with_ignore_case(c, last_token) {
                                        if match_count < 16 {
                                            matches[match_count] = c;
                                            match_count += 1;
                                        }
                                    }
                                }
                                ...
                                if match_count == 1 {
                                    ...
                                    if term.input_len < term.input_buf.len() && !c.contains('\\') {
                                        term.input_buf[term.input_len] = b' ';
                                        term.input_len += 1;
                                    }
                                } else if match_count > 1 {
                                    // Cycle through available matches
                                    static mut CYCLE_IDX: usize = 0;
                                    let cur_idx = unsafe {
                                        let idx = CYCLE_IDX % match_count;
                                        CYCLE_IDX += 1;
                                        idx
                                    };

                                    let c = matches[cur_idx];
                                    term.input_len = prefix_len;
                                    for byte in c.bytes() {
                                        if term.input_len < term.input_buf.len() {
                                            term.input_buf[term.input_len] = byte;
                                    ...
                                    term.add_line("[Tab: Press Tab to cycle next option]");
                                }
